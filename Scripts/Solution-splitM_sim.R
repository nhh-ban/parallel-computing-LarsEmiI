...
MTweedieTests <- function(N, M, sig) {
  results <- future_map_dbl(1:M, ~simTweedieTest(N) < sig) # Parallellization on MTweedieTests function
  
  
  share_reject <- mean(results) # Calculate the proportion of simulations that are less than sig
  
  return(share_reject)
}

# Redefine df as previously
df <- expand.grid(
...
# Running simulations using the parallelized MTweedieTests function, otherwise its the same as original script
for (i in 1:nrow(df)) {
